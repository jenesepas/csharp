    ...
    public partial class MRegistros : Form
    {
        ...
        private void btt_guardar_rg_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                pRegistro.et_tasa = Convert.ToInt32(tb_tasa.Text);  //float.Parse(tb_tasa.Text);
                ...
                pRegistro.et_tasa2 = Convert.ToInt32(tb_tasa2.Text);  
                pRegistro.t_tasa2 = tb_tipo2.Text.Trim();
                pRegistro.et_tasa3 = Convert.ToInt32(tb_tasa3.Text);  
                pRegistro.t_tasa3 = tb_tipo3.Text.Trim();
                pRegistro.et_tasa4 = Convert.ToInt32(tb_tasa4.Text);  
        ...
        private void btt_modificar_rg_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                pRegistro.fec_fra = Convert.ToDateTime(dtp_fec_fra.Text);
                pRegistro.observacion = tb_observ.Text.Trim();
                pRegistro.honorarios = decimal.Parse(tb_hono.Text); //Convert.ToDecimal(tb_base.Text);
                pRegistro.p_iva = Convert.ToInt16(tb_p_iva.Text);
                pRegistro.tasa = decimal.Parse(tb_tasa_fra.Text); //Convert.ToDecimal(tb_tasa_fra.Text);
                pRegistro.exp_tl = tb_exp.Text.Trim();
                pRegistro.fec_pre_exp = Convert.ToDateTime(dtp_fec_pre.Text);
                pRegistro.et_tasa = Convert.ToInt32(tb_tasa.Text);
                pRegistro.t_tasa = tb_tipo.Text.Trim();
                pRegistro.cambio_serv = tb_c_serv.Text.Trim();
                pRegistro.bate_ant = tb_bate_ant.Text.Trim();
                pRegistro.nif = tb_nif.Text.Trim();
        ...
        {                        
            ...
            tb_d_col.Text = "0";
            cb_cte_fra.Text = "CLIENTE";
            tb_base_imp.Text = (Convert.ToDecimal(tb_hono.Text) + Convert.ToDecimal(tb_d_col)).ToString("N2");
            tb_t_fra.Text = Reg_Opera.Calcular_tot_fra(tb_base_imp.Text, tb_p_iva.Text, tb_tasa_fra.Text);
            tb_tasa2.Text = "0";
            tb_tipo2.Text = " ";
            tb_tasa3.Text = "0";
        ...
        {
            if (!string.IsNullOrWhiteSpace(tb_pdf.Text))
            {
                //string filePath = "@\"" + tb_pdf.Text + "\"";
                Process.Start(tb_pdf.Text);
            }
            else
                MessageBox.Show("Necesita adjuntar un fichero pdf.","Atención",MessageBoxButtons.OK,MessageBoxIcon.Stop);
        }

        private void tb_descrip_Enter(object sender, EventArgs e)
        {
            tb_descrip.Text = cb_sec.Text.Trim() + " - " + cb_tram.Text.Trim();
        }

        private void tb_tasa2_Validating(object sender, CancelEventArgs e)
        {
            //validamos que sea entero
            if (General.Validar_entero(tb_tasa2.Text) == -1)
            {
                tb_tasa2.Focus();
                tb_tasa2.SelectAll();
            }
        ...
        {
            //validamos que sea entero
            if (General.Validar_entero(tb_tasa3.Text) == -1)
            {
                tb_tasa3.Focus();
                tb_tasa3.SelectAll();
            }
        ...
        {
            //validamos que sea entero
            if (General.Validar_entero(tb_tasa4.Text) == -1)
            {
                tb_tasa4.Focus();
                tb_tasa4.SelectAll();
            }
