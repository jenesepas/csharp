...
namespace Asegest
{
    public partial class MClientes : Form
    {
        ...
        private void MClientes_Load(object sender, EventArgs e)
        {
            ...
            //consulta desde Mregistros para ver algun dato + del Cte.
        ...
        private void Asigna_campos_cte(Cliente pcliente)
        {
            ...
            tb_cta.Text = pcliente.cta_cble;

            if (pcliente.Tipo_cte == 'C')
                rb_cte.Checked = true;
            else
            {
                if (pcliente.Tipo_cte == 'T')
        ...
        {
            //hacemos comprobacion solo si no es vacio y cambia el valor anterior
            if (tb_documento.Text.Trim() != "" & tb_documento.Text != docu_old)
            {
                ...
                {                    
                    if (Ctes_Opera.Existe_dni_cte(tb_documento.Text) > 0) //Si > 0, ya existe.            
                        MessageBox.Show("Ya existe un dni/cif asignado a otro cliente!", "DNI/CIF repetido", MessageBoxButtons.OK, MessageBoxIcon.Warning);                
                }
            }
        }
        ...
        private void tb_cta_Validating(object sender, CancelEventArgs e)
        {
            if (General.Validar_long_exacta(tb_cta.Text.TrimEnd(), 9) == -1)
            {
                tb_cta.Focus();
                tb_cta.Text = "430000000";
                tb_cta.SelectAll();
        ...
        private void tb_documento_Enter(object sender, EventArgs e)
        {
             docu_old= tb_documento.Text; //sabemos el valor anterior
            //tb_fra.CausesValidation = true; //activamos que pase por validating
        }

        private void tb_documento_Validated(object sender, EventArgs e)
        {
            //hacemos comprobacion solo si no es vacio y cambia el valor anterior
            if (!string.IsNullOrWhiteSpace(tb_documento.Text) & tb_documento.Text != docu_old)
            {                
